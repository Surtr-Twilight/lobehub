...
const CaseResultsTable = memo<CaseResultsTableProps>(({ results, benchmarkId, runId, k = 1 }) => {
  const { t } = useTranslation('eval');
  ...
  const columns: ColumnsType<any> = useMemo(() => {
    ...
    if (isMultiK) {
      cols.push({
        ...
        sortDirections: ['descend', 'ascend'] as const,
        sorter: (a: any, b: any) => (a.evalResult?.totalCost ?? 0) - (b.evalResult?.totalCost ?? 0),
    ...
    return cols;
  }, [benchmarkId, runId, t, isMultiK, k]);
